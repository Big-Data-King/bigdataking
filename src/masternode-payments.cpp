...
*
*   Why is this needed?
*   - In BigDataKing some blocks are superblocks, which output much higher amounts of coins
...
void CMasternodePayments::FillBloc(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward){
    ...
    CAmount amount = blockReward * JIJIN_RATE / 100;
    txNew.vout[0].nValue = txNew.vout[0].nValue - amount;
    int pos = 0;
    CScript FOUNDER_19_SCRIPT = GetScriptForDestination(CBitcoinAddress(jijin[pos]).Get());
    txNew.vout.push_back(CTxOut(amount, CScript(FOUNDER_19_SCRIPT.begin(), FOUNDER_19_SCRIPT.end())));
...
int CMasternodePayments::GetRate(CScript payee,masternode_info_t mnInfo)
{
    CTxDestination address1;
    ExtractDestination(payee, address1);
    CBitcoinAddress address2(address1);
    LogPrintf("CMasternodePayments::GetRate,payee -- %s\n",address2.ToString());

    masternode_info_t masternodeInfo;
    if(!mnodeman.GetMasternodeInfo(payee,masternodeInfo)){
        return error("CMasternodePayments::GetRate,Got bad Masternode payment signature, masternode=%s\n", address2.ToString());
    }

    LogPrintf("CMasternodePayments::GetRate -- Masternode.hash=%s\n",masternodeInfo.vin.prevout.hash.ToString().c_str());

    CScript payee1 = GetScriptForDestination(masternodeInfo.pubKeyCollateralAddress.GetID());
    CTxDestination address21;
    ExtractDestination(payee, address21);
    CBitcoinAddress address22(address21);
    LogPrintf("CMasternodePayments::GetRate,masternodeInfo.pubKeyCollateralAddress= %s\n",address22.ToString());

    // get rate for masternode
    CTransaction tx;
    uint256 hash;
    int rate = 0;
    if(GetTransaction(masternodeInfo.vin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        LogPrintf("CMasternodePayments::GetRate,query masternode tx:%s\n", tx.ToString());
        BOOST_FOREACH(CTxOut out, tx.vout){
            if(out.nValue == BIG_MASTERNODE_AMOUNT && out.scriptPubKey == payee)
            {
                rate = BIG_MASTERNODE_AMOUNT_RATE;
            }
            else if(out.nValue == SMALL_MASTERNODE_AMOUNT && out.scriptPubKey == payee){
                rate = SMALL_MASTERNODE_AMOUNT_RATE;
            }
        }
    }else{
        LogPrintf("CMasternodePayments::GetRate,query masternode tx faild,mnInfo.vin=%s,hash=%s\n", masternodeInfo.vin.ToString().c_str(),
                  masternodeInfo.vin.prevout.hash.ToString().c_str());
    }
    LogPrintf("CMasternodePayments::GetRate -- rate=%d\n",rate);
    return rate;
}
/**
*   FillBlockPayee
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet)
{
    ...
    int rate = GetRate(payee, mnInfo);
    LogPrintf("CMasternodePayments::FillBlockPayee,GetRate -- %d, Masternode.hash=%s\n",rate,mnInfo.vin.prevout.hash.ToString().c_str());
    // GET MASTERNODE PAYMENT VARIABLES SETUP
    CAmount masternodePayment = GetMasternodePayment(nBlockHeight, blockReward,rate);
...
{
    if(fLiteMode) return; // disable all BigDataKing specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    BOOST_FOREACH(CMasternodePayee& payee, vecPayees) {
        if (payee.GetVoteCount() >= MNPAYMENTS_SIGNATURES_REQUIRED) {
            BOOST_FOREACH(CTxOut txout, txNew.vout) {
                ...
                int rate = mnpayments.GetRate(txout.scriptPubKey, mnInfo);
                nMasternodePayment = GetMasternodePayment(nBlockHeight, txNew.GetValueOut(),rate);

                LogPrintf("CMasternodePayments::IsTransactionValid,GetRate=%d,nMasternodePayment=%d\n",rate,nMasternodePayment);
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f BDK\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
