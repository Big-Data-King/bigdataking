...
const char * const BITCOIN_CONF_FILENAME = "BigDataKing.conf";
const char * const BITCOIN_PID_FILENAME = "BigDataKingd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "BigDataKing" is a composite category enabling all BigDataKing-related debug output
            if(ptrCategory->count(string("BigDataKing"))) {
...
#else
    const char* pszModule = "BigDataKing";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\bigdataking
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\bigdataking
    // Mac: ~/Library/Application Support/bigdataking
    // Unix: ~/.bigdataking
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bigdataking";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BigDataKingCore";
#else
    // Unix
    return pathRet / ".bigdataking";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty BigDataKing.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override BigDataKing.conf
