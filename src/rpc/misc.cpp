...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total BigDataKing balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized BigDataKing balance of the wallet\n"
...
UniValue debug(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "BigDataKing|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use comma to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "BigDataKing")
            + HelpExampleRpc("debug", "BigDataKing,net")
        );
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"BigDataKingaddress\"\n"
            "\nReturn information about the given BigDataKing address.\n"
            "\nArguments:\n"
            "1. \"BigDataKingaddress\"     (string, required) The BigDataKing address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"BigDataKingaddress\", (string) The BigDataKing address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: BigDataKing address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are BigDataKing addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) BigDataKing address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"BigDataKingaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"BigDataKingaddress\"  (string, required) The BigDataKing address to use for the signature.\n"
