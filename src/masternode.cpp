...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, int& nHeightRet)
{
    ...
    if(!(coin.out.nValue == BIG_MASTERNODE_AMOUNT || coin.out.nValue == SMALL_MASTERNODE_AMOUNT)) {
...
bool CMasternode::IsInputAssociatedWithPubkey()
{
    ...
    if(GetTransaction(vin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        BOOST_FOREACH(CTxOut out, tx.vout){
            LogPrintf("CMasternode::IsInputAssociatedWithPubkey(),CTxOut=%s\n", out.ToString());
            LogPrintf("CMasternode::IsInputAssociatedWithPubkey(),out.nValue == BIG_MASTERNODE_AMOUNT || out.nValue == SMALL_MASTERNODE_AMOUNT=%s\n", out.nValue == BIG_MASTERNODE_AMOUNT || out.nValue == SMALL_MASTERNODE_AMOUNT);
            if((out.nValue == BIG_MASTERNODE_AMOUNT || out.nValue == SMALL_MASTERNODE_AMOUNT) && out.scriptPubKey == payee) return true;
        }
    }
...
void CMasternode::UpdateLastPaid(const CBlockIndex *pindex, int nMaxBlocksToScanBack)
{
    ...
    for (int i = 0; BlockReading && BlockReading->nHeight > nBlockLastPaid && i < nMaxBlocksToScanBack; i++) {
            ...
            mnpayments.mapMasternodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(mnpayee, 2))
        {
            ...
            CAmount nMasternodePayment = GetMasternodePayment(BlockReading->nHeight, block.vtx[0].GetValueOut(),rate);
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if (err == COLLATERAL_INVALID_AMOUNT) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 1000 BDK, masternode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 1000 BDK tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 1000 BDK tx -> 1 confirmation
