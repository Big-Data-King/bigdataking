...
#if defined(NDEBUG)
# error "BigDataKing Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if((nPrevHeight+1) % 1000 ==0){
        nSubsidy = 50000*COIN;
    }else if((nPrevHeight+1) % 100 ==0){
        nSubsidy = 20000*COIN;
    }else{
        nSubsidy = 5000*COIN;
    }
...
//{
//    CAmount nSubsidy = 0; // start at 20%
//
//    if(nHeight < 262800){
//        nSubsidy = 10*COIN;
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue,int64_t rate)
{
    return blockValue * rate /100;
}
...
void ThreadScriptCheck() {
    RenameThread("BigDataKing-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // BDK : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(BDK): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(BDK): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // BDK : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(BDK): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(BDK): spork is off, skipping transaction locking checks\n");
    }
