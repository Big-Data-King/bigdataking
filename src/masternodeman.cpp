...
bool CMasternodeMan::GetNextMasternodeInQueueForPayment(int nBlockHeight, bool fFilterSigTime, int& nCountRet, masternode_info_t& mnInfoRet)
{
    ...
    */
    ...
    for (auto& mnpair : mapMasternodes) {
        ...
        LogPrintf("CMasternodeMan::GetNextMasternodeInQueueForPayment --seconds=%d", seconds);
        if(seconds < time_interval) {
            LogPrintf("CMasternodeMan::GetNextMasternodeInQueueForPayment -- masternode: addr=%s， seconds=%d, time interval=%lli,can't add to mapMasternodes\n",
                      mnpair.second.addr.ToString(), seconds, time_interval);
            continue;
        }
